...
class VNGenerator:
    ...
    def generate(self, user_input: str, output_base_path: str = "./output",
                 font_name: str = "auto") -> GenerationResult:
        print("🎬 阶段1: 生成剧本...")
        try:
    ...
    def _normalize_code_result(self, raw: Any) -> dict:
        ...
        if isinstance(output, dict) and "files" in output and isinstance(output["files"], list):
            return {
                "files": output["files"],
                "assets_needed": output.get("assets_needed", raw.get("assets_needed", []))
            }
        ...
        if isinstance(data, dict) and "files" in data and isinstance(data["files"], list):
            return {
                "files": data["files"],
                "assets_needed": data.get("assets_needed", raw.get("assets_needed", []))
            }
        ...
        if "path" in raw and "content" in raw:
            return {
                "files": [{"path": raw["path"], "content": raw["content"]}],
                "assets_needed": raw.get("assets_needed", [])
            }
        ...
        for key, value in raw.items():
            if isinstance(value, dict) and "files" in value and isinstance(value["files"], list):
                return {
                    "files": value["files"],
                    "assets_needed": value.get("assets_needed", raw.get("assets_needed", []))
                }
    ...
    def _normalize_assets_needed(self, assets_needed: Any) -> list[dict]:
        ...
        for item in assets_needed:
            if isinstance(item, str):
                normalized.append({
                    "path": item,
                    "type": self._infer_asset_type(item)
                })
                continue
